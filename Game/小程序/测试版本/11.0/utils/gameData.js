...
function plantCrop(landId, cropType) {
  ...
  if (land.purchased === false) return { success: false, msg: '请先在商店购买该土地' };
  ...
  land.isMutated = false;
  land.mutationInfo = null;

  // 变异判定
  if (config.mutation && Math.random() < config.mutation.chance) {
    land.isMutated = true;
    land.mutationInfo = {
      name: config.mutation.name,
      multiplier: config.mutation.multiplier,
      emoji: config.mutation.emoji
    };
  }
  ...
  if (bonuses.growthBoost) growTime = growTime * (1 - bonuses.growthBoost);
  if (bonuses.instantGrow) growTime = 1;
  land.growTime = Math.max(1, Math.floor(growTime));
  land.watered = false;

  // 自动喷灌器
  if (data.tools && data.tools.sprinkler) {
    land.watered = true;
...
function harvestLand(landId) {
  ...
  // 变异加成
  var isMutated = land.isMutated && land.mutationInfo;
  var cropDisplayName = config.name;
  if (isMutated) {
    price = Math.floor(price * land.mutationInfo.multiplier);
    expReward = Math.floor(expReward * land.mutationInfo.multiplier);
    cropDisplayName = land.mutationInfo.emoji + land.mutationInfo.name;
  }

  data.gold += price;
  data.totalGoldEarned += price;
  data.harvestCount += 1;

  if (!data.inventory) data.inventory = {};
  data.inventory[land.cropType] = (data.inventory[land.cropType] || 0) + 1;

  var harvestGain = 0;
  while (data.harvestCount >= data.harvestMilestone) {
    data.skillPoints += 1;
    harvestGain += 1;
    data.harvestMilestone += 15;
  }
  ...
  var milestoneMsg = '';
  if (harvestGain > 0) milestoneMsg += '收获 ' + harvestGain + ' 点技能';
  if (goldGain > 0) milestoneMsg += (milestoneMsg ? '，' : '') + '金币 ' + goldGain + ' 点技能';
  if (milestoneMsg) milestoneMsg = '（' + milestoneMsg + '）';
  ...
  save();
  return { success: true, land: land, reward: price, expReward: expReward, cropName: cropDisplayName, isMutated: isMutated, leveledUp: leveledUp, newLevel: data.level, milestoneMsg: milestoneMsg };
}
