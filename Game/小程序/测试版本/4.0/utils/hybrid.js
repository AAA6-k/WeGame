...
function breed(landIdA, landIdB) {
  ...
  // 首次发现该杂交配方，奖励技能点
  var recipeKey = [landA.cropType, landB.cropType].sort().join('_');
  var isNewDiscovery = false;
  if (data.hybridDiscovered.indexOf(recipeKey) === -1) {
    data.hybridDiscovered.push(recipeKey);
    data.skillPoints += 1;
    isNewDiscovery = true;
  }

  app.saveData();
  return { success: true, hybrid: result, landId: landIdA, cropKey: newCropKey, isNewDiscovery: isNewDiscovery };
}
